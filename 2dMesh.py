...
print("Stats: Total Tx Cnt: %d, Total Rx Cnt: %d, Total Pend Cnt: %d" %
      (totalTxCnt, totalRxCnt, totalPendCnt))
print("Average Latency in clocks:%.2f, Maximum Latency: %d " %
      (totalLatency/totalRxCnt, maxLatency))
print("Average Hop count :%.2f, Maximum Hop Count: %d" %
      (totalHopCnt/totalRxCnt, maxHopCnt))
...
print("Average Packet Latency Per source ", list(
    map(lambda x, y: round(div(x, y), 2), rcvdLatPerSource, rcvdPktPerSource)))
for x in range(xMax):
    for y in range(yMax):
...
for x in range(xMax):
    for y in range(yMax):
        ...
        for dir in dirA:
            ...
            myGridImage.line_write(
                myNum, dir, changeColor, cntArray[x][y][dir])
